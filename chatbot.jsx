...
export default function Chatbot() {
  ...
  const submitCorrection = async () => {
    ...
    const correctedMsg = messages.find(m => m.id === correctingId);
    const lesson = `Correction: When asked "${messages[messages.findIndex(m => m.id === correctingId) - 1]?.content}", the previous answer was wrong. The correct answer is: "${correctionText}"`;
  ...
  return (
    <div style={{
      ...
      {memory.length > 0 && (
        <div style={{
          width: "100%", maxWidth: 720, marginBottom: 16,
      ...
      <div style={{
        ...
        background: "#0b1318", border: "1px solid #1a2a20",
        borderRadius: 14, padding: 20, minHeight: 400, maxHeight: "60vh",
          ...
          <div key={msg.id} style={{ animation: "fadeUp 0.3s ease" }}>
            <div style={{
              ...
              <div style={{
                ...
                background: msg.role === "user" ? "#1a3a5a" : "#1a3a2a",
                border: `1px solid ${msg.role === "user" ? "#2a5a8a" : "#2a6a4a"}`,
              ...
              <div style={{ maxWidth: "80%" }}>
                <div style={{
                  background: msg.role === "user" ? "#0e1f30" : "#0e1f18",
                  border: `1px solid ${msg.role === "user" ? "#1a3a5a" : "#1a3a2a"}`,
                  borderRadius: msg.role === "user" ? "14px 4px 14px 14px" : "4px 14px 14px 14px",
                  padding: "10px 14px", fontSize: 14, color: "#c8e8d8",
                  lineHeight: 1.6, whiteSpace: "pre-wrap"
                }}>
                  {corrections[msg.id] && (
                    <div style={{ fontSize: 10, color: "#5a8a6a", marginBottom: 6, letterSpacing: 1 }}>
                      ✅ CORRECTED
                    </div>
                  )}
                  {msg.content}
                      ...
                      <div style={{ marginTop: 8, animation: "fadeUp 0.2s ease" }}>
                        <textarea
                          ...
                          style={{
                            ...
                            color: "#a8d8b8", fontSize: 13, padding: "8px 12px",
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                          <button onClick={submitCorrection} style={{
                            ...
                            borderRadius: 6, padding: "4px 14px", color: "#7ee8a2",
                            cursor: "pointer", fontSize: 12, fontFamily: "monospace"
                          }}>Submit</button>
                          <button onClick={() => setCorrectingId(null)} style={{
                            background: "transparent", border: "1px solid #334",
                            borderRadius: 6, padding: "4px 14px", color: "#567",
                            cursor: "pointer", fontSize: 12, fontFamily: "monospace"
                          }}>Cancel</button>
                        </div>
                      </div>
      ...
      <div style={{
        ...
        <textarea
          ...
          style={{
            ...
            color: "#c8e8d8", fontSize: 14, fontFamily: "monospace",
        ...
        <button
          ...
          style={{
            background: input.trim() && !loading ? "#1a4a2a" : "#0e1e18",
            border: `1px solid ${input.trim() && !loading ? "#2a6a3a" : "#1a2a1a"}`,
            borderRadius: 8, width: 36, height: 36, cursor: input.trim() && !loading ? "pointer" : "default",
            fontSize: 16, transition: "all 0.2s", flexShrink: 0,
            color: input.trim() && !loading ? "#7ee8a2" : "#2a4a3a"
          }}>
          ➤
        </button>
      </div>
